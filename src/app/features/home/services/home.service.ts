...
export class HomeService {
  ...
  getGroupsAndEventsByLocationYear(
    lat: number,
    lng: number,
    year: number
  ): Observable<{ groups: groupDetailsModel[]; locationLabel?: string }> {
    const params = {
      lat: lat.toString(),
      lng: lng.toString(),
      year: year.toString()
    ...
    return this.http.get<{ groups: groupDetailsModel[]; locationLabel?: string }>(this.apiUrl, { params }).pipe(
      catchError(() => of({ groups: this.filterLocalGroupsByYear(year) }))
    );
  }

  private filterLocalGroupsByYear(year: number): groupDetailsModel[] {
    return this.samitiGroups.map(group => ({
      ...group,
